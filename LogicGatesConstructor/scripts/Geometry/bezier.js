...
import { RectBounds } from "./rect_bounds.js";

const THRESHOLD_DIST = 1e-3
...
export const cubic_bezier = (t, p1, p2, p3, p4) => {
  ...
  const t4 = t * t * t;
  return new Vector2d(
    p1.x * t1 + p2.x * t2 + p3.x * t3 + p4.x * t4,
...
const cubic_bezier_bounds_x = (p1, p2, p3, p4) => {
  ...
  if (Math.abs(a) < THRESHOLD_DIST) {
    if (Math.abs(b) < THRESHOLD_DIST)
      return new Vector2d(Math.min(p1.x, p4.x), Math.max(p1.x, p4.x));
    const x = cubic_bezier_x(-c / b, p1, p2, p3, p4);
    return new Vector2d(
      Math.min(Math.min(x, p1.x), p4.x),
      Math.max(Math.max(x, p1.x), p4.x)
    );
  }
  const d = b * b - a * c * 4.0;
  if (d <= 0) return new Vector2d(Math.min(p1.x, p4.x), Math.max(p1.x, p4.x));
  const sqd = Math.sqrt(d);
  const x_0 = cubic_bezier_x(
    Math.min(Math.max(0.0, (-sqd - b) / (2.0 * a)), 1.0),
    p1,
    p2,
    p3,
    p4
  );
  const x_1 = cubic_bezier_x(
    Math.min(Math.max(0.0, (sqd - b) / (2.0 * a)), 1.0),
    p1,
    p2,
    p3,
  ...
  return new Vector2d(
    Math.min(Math.min(Math.min(p1.x, p4.x), x_0), x_1),
...
const cubic_bezier_bounds_y = (p1, p2, p3, p4) => {
  ...
  if (Math.abs(a) < THRESHOLD_DIST) {
    if (Math.abs(b) < THRESHOLD_DIST)
      return new Vector2d(Math.min(p1.y, p4.y), Math.max(p1.y, p4.y));
    const y = cubic_bezier_y(-c / b, p1, p2, p3, p4);
    return new Vector2d(
      Math.min(Math.min(y, p1.y), p4.y),
      Math.max(Math.max(y, p1.y), p4.y)
    );
  }
  const d = b * b - a * c * 4.0;
  if (d <= 0) return new Vector2d(Math.min(p1.y, p4.y), Math.max(p1.y, p4.y));
  const sqd = Math.sqrt(d);
  const y_0 = cubic_bezier_y(
    Math.min(Math.max(0.0, (-sqd - b) / (2.0 * a)), 1.0),
    p1,
    p2,
    p3,
  ...
  return new Vector2d(
    Math.min(Math.min(Math.min(p1.y, p4.y), y_0), y_1),
...
export const cubic_bezier_bounds = (p1, p2, p3, p4) => {
  ...
  return new RectBounds(
    new Vector2d(min_max_x.x, min_max_y.x),
    new Vector2d(min_max_x.y, min_max_y.y)
  );
};
...
const is_close_to_segment = (point, p1, p2, threshold = 1.0) => {
  ...
  const dist = Math.sqrt(
    Math.pow(point.x - c * d.x, 2) + Math.pow(point.y - c * d.y, 2)
  );
...
export const is_close_to_bezier = (point, p1, p2, p3, p4, threshold = 1.0) => {
  for (const [t1, t2] of BEZIER_SECTIONS_PARAM_T) {
    ...
    if (is_close_to_segment(point, bezier_p1, bezier_p2, threshold))
      return true;
  }
  return false;
};
