...
class Transform2d
{
	...
	get sync_transform() { return is_bit_set(this.#transform_status, SYNC_TRANSFORM_BIT); }
	set sync_transform(value) { this.#transform_status = value ? set_bit(this.#transform_status, SYNC_TRANSFORM_BIT)
																:clear_bit(this.#transform_status, SYNC_TRANSFORM_BIT); }
	#set_raw()
	{
		this.raw_transform = true;
		this.sync_transform = true;
	}
	sync()
	{
		// if(!this.sync_transform)return;
		this.sync_transform = false;
		if(!this.has_parent)
		{
